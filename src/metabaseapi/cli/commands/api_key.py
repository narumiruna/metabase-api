...
def create_api_key(ctx: typer.Context, body: str = typer.Argument(..., help="API key JSON object")) -> None:
    run_json_body_endpoint_command(ctx, body, lambda payload: CreateApiKeyRequest(body=payload))
...
def update_api_key(ctx: typer.Context, api_key_id: str = typer.Argument(...), body: str = typer.Argument(...)) -> None:
    run_json_body_endpoint_command(ctx, body, lambda payload: UpdateApiKeyRequest(api_key_id=api_key_id, body=payload))
