...
def create_api_key(ctx: typer.Context, body: str = typer.Argument(..., help="API key JSON object")) -> None:
    ...
    _run_and_print(_run_client_call(ctx, lambda client: _raw_api_key.create_api_key(client, payload)))
...
def list_api_keys(ctx: typer.Context) -> None:
    _run_and_print(
        _run_client_call(
            ctx,
            lambda client: _raw_api_key.list_api_keys(
                client,
            ),
        )
    )
...
def count_api_keys(ctx: typer.Context) -> None:
    _run_and_print(
        _run_client_call(
            ctx,
            lambda client: _raw_api_key.count_api_keys(
                client,
            ),
        )
    )
...
def update_api_key(ctx: typer.Context, api_key_id: str = typer.Argument(...), body: str = typer.Argument(...)) -> None:
    payload = _parse_json_object(body, "body")
    _run_and_print(_run_client_call(ctx, lambda client: _raw_api_key.update_api_key(client, api_key_id, payload)))


@app.command("delete-api-key")
def delete_api_key(ctx: typer.Context, api_key_id: str = typer.Argument(...)) -> None:
    _run_and_print(_run_client_call(ctx, lambda client: _raw_api_key.delete_api_key(client, api_key_id)))


@app.command("regenerate-api-key")
def regenerate_api_key(ctx: typer.Context, api_key_id: str = typer.Argument(...)) -> None:
    _run_and_print(_run_client_call(ctx, lambda client: _raw_api_key.regenerate_api_key(client, api_key_id)))
