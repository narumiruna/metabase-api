...
Completed: 2026-05-17 02:27:26Z
...
- `[x]` = this repository can call this documented endpoint today.
- Section headings keep the explicit-helper count (`focused helpers / documented operations`) so future work can choose high-value endpoints for focused typed models and convenience commands.
- Raw-only endpoints are complete through `MetabaseClient.request(...)` and `metabaseapi request|invoke METHOD PATH`; do not add generated endpoint modules, runtime endpoint registries, or file-scanning endpoint discovery.
- Add focused models and convenience commands by hand only when an endpoint is high-value enough to justify a deeper interface.

## Coverage summary

- Documented operations: 600
- Completed documented operations: 600
- Explicit hand-written endpoint helpers: 16
- Raw-only completed operations: 584
- Raw coverage: all documented operations use `DELETE`, `GET`, `PATCH`, `POST`, or `PUT`, which are supported by the generic raw request path.

## Local drift resolved

- [x] Removed the out-of-doc `GET /api/field` list convenience after live verification returned 404; documented field support now starts at `GET /api/field/{id}`.

## Endpoint checklist

### /api/action (0/10 explicit)
...
### /api/ai-entity-analysis (0/1 explicit)
...
### /api/automagic-dashboards (0/11 explicit)
...
### /api/bug-reporting (0/2 explicit)
...
### /api/cloud-migration (0/3 explicit)
...
- [x] `GET /api/collection/root/items` — `get-api-collection-root-items` — Fetch objects that the current user should see at their root level. As mentioned elsewhere, the 'Root' Collection doesn't actually exist as a row in the appl...
...
### /api/data-studio/table (0/5 explicit)
...
- [x] `GET /api/database/field-values` — `get-api-database-field-values` — Get sampled field values for every field in the instance, streamed as a single `{"field_values": [...]}` document. Each entry carries `field_id`, `values`, o...
- [x] `GET /api/database/metadata` — `get-api-database-metadata` — Get metadata (databases, tables, and fields) for all databases visible to the current user. Returns a flat structure with three arrays: databases, tables, an...
- [x] `POST /api/database/metadata` — `post-api-database-metadata` — Import database/table/field metadata previously exported from `GET /api/database/metadata`. Entities are matched by natural key — databases by `(name, engine...
...
- [x] `GET /api/database/{id}/autocomplete_suggestions` — `get-api-database-id-autocomplete_suggestions` — Return a list of autocomplete suggestions for a given `prefix`, or `substring`. Should only specify one, but `substring` will have priority if both are prese...
- [x] `GET /api/database/{id}/card_autocomplete_suggestions` — `get-api-database-id-card_autocomplete_suggestions` — Return a list of `Card` autocomplete suggestions for a given `query` in a given `Database`. This is intended for use with the ACE Editor when the User is typ...
- [x] `POST /api/database/{id}/discard_values` — `post-api-database-id-discard_values` — Discards all saved field values for this `Database`.
- [x] `POST /api/database/{id}/dismiss_spinner` — `post-api-database-id-dismiss_spinner` — Manually set the initial sync status of the `Database` and corresponding tables to be `complete` (see #20863)
- [x] `GET /api/database/{id}/fields` — `get-api-database-id-fields` — Get a list of all `Fields` in `Database`.
- [x] `GET /api/database/{id}/healthcheck` — `get-api-database-id-healthcheck` — Reports whether the database can currently connect
- [x] `GET /api/database/{id}/idfields` — `get-api-database-id-idfields` — Get a list of all primary key `Fields` for `Database`.
- [x] `GET /api/database/{id}/metadata` — `get-api-database-id-metadata` — Get metadata about a `Database`, including all of its `Tables` and `Fields`. Returns DB, fields, and field values. By default only non-hidden tables and fiel...
- [x] `POST /api/database/{id}/rescan_values` — `post-api-database-id-rescan_values` — Trigger a manual scan of the field values for this `Database`.
- [x] `GET /api/database/{id}/schema` — `get-api-database-id-schema` — Return a list of Tables for a Database whose `schema` is `nil` or an empty string. Optional filters: - `can-query=true` - filter to only tables the user can...
- [x] `GET /api/database/{id}/schema/{schema}` — `get-api-database-id-schema-schema` — Returns a list of Tables for the given Database `id` and `schema`. Optional filters: - `can-query=true` - filter to only tables the user can query - `can-wri...
- [x] `GET /api/database/{id}/schemas` — `get-api-database-id-schemas` — Returns a list of all the schemas with tables found for the database `id`. Excludes schemas with no tables. Optional filters: - `can-query=true` - filter to...
- [x] `GET /api/database/{id}/settings-available` — `get-api-database-id-settings-available` — Get all database-local settings and their availability for the given database.
- [x] `POST /api/database/{id}/sync_schema` — `post-api-database-id-sync_schema` — Trigger a manual update of the schema metadata for this `Database`.
- [x] `GET /api/database/{id}/syncable_schemas` — `get-api-database-id-syncable_schemas` — Returns a list of all syncable schemas found for the database `id`.
- [x] `GET /api/database/{id}/usage_info` — `get-api-database-id-usage_info` — Get usage info for a database. Returns a map with keys are models and values are the number of entities that use this database.
- [x] `GET /api/database/{virtual-db}/datasets` — `get-api-database-virtual-db-datasets` — Returns a list of all the datasets found for the saved questions virtual database.
- [x] `GET /api/database/{virtual-db}/datasets/{schema}` — `get-api-database-virtual-db-datasets-schema` — Returns a list of Tables for the datasets virtual database.
- [x] `GET /api/database/{virtual-db}/metadata` — `get-api-database-virtual-db-metadata` — Endpoint that provides metadata for the Saved Questions 'virtual' database. Used for fooling the frontend and allowing it to treat the Saved Questions virtua...
- [x] `GET /api/database/{virtual-db}/schema/{schema}` — `get-api-database-virtual-db-schema-schema` — Returns a list of Tables for the saved questions virtual database.
- [x] `GET /api/database/{virtual-db}/schemas` — `get-api-database-virtual-db-schemas` — Returns a list of all the schemas found for the saved questions virtual database.

### /api/dataset (0/8 explicit)
...
### /api/ee/advanced-permissions/application (0/2 explicit)
...
### /api/ee/advanced-permissions/impersonation (0/2 explicit)
...
### /api/ee/ai-controls/permissions (0/2 explicit)
...
### /api/ee/ai-controls/usage (0/8 explicit)
...
### /api/ee/audit-app/analytics-dev (0/1 explicit)
...
### /api/ee/audit-app/user (0/2 explicit)
...
### /api/ee/cloud-add-ons (0/4 explicit)
...
### /api/ee/cloud-proxy (0/1 explicit)
...
### /api/ee/content-translation (0/4 explicit)
...
### /api/ee/data-complexity-score (0/1 explicit)
...
### /api/ee/data-studio/table (0/2 explicit)
...
### /api/ee/database-replication (0/3 explicit)
...
### /api/ee/database-routing (0/2 explicit)
...
### /api/ee/dependencies (0/9 explicit)
...
### /api/ee/embedding-hub (0/1 explicit)
...
### /api/ee/remote-sync (0/11 explicit)
...
### /api/ee/replacement (0/6 explicit)
...
### /api/ee/security-center (0/5 explicit)
...
### /api/ee/semantic-search (0/1 explicit)
...
### /api/ee/serialization (0/2 explicit)
...
### /api/ee/support-access-grant (0/4 explicit)
...
### /api/ee/transforms (0/3 explicit)
...
### /api/ee/transforms-python (0/3 explicit)
...
### /api/ee/upload-management (0/2 explicit)
...
- [x] `GET /api/embed/pivot/dashboard/{token}/dashcard/{dashcard-id}/card/{card-id}` — `get-api-embed-pivot-dashboard-token-dashcard-dashcard-id-card-card-id` — Fetch the results of running a Card belonging to a Dashboard using a JSON Web Token signed with the `embedding-secret-key`
...
### /api/frontend-errors (0/1 explicit)
...
### /api/metabot/document (0/1 explicit)
...
### /api/metabot/metabot (0/7 explicit)
...
### /api/metabot/permissions (0/1 explicit)
...
### /api/metabot/slack (0/3 explicit)
...
### /api/native-query-snippet (0/4 explicit)
...
### /api/notification/unsubscribe (0/2 explicit)
...
### /api/premium-features (0/2 explicit)
...
### /api/preview_embed (0/13 explicit)
...
### /api/pulse/unsubscribe (0/2 explicit)
...
### /api/timeline-event (0/4 explicit)
...
### /api/transform-job (0/7 explicit)
...
### /api/transform-tag (0/4 explicit)
...
### /api/user-key-value (0/4 explicit)
