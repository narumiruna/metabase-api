...
Generated: 2026-05-17 02:20:54Z
...
- `[x]` = this repository already has explicit hand-written support for this endpoint through focused client/CLI/typed helpers.
- `[ ]` = this endpoint is currently raw-only. It is still callable with `MetabaseClient.request(...)` or `metabaseapi request|invoke METHOD PATH`, but it does not yet have focused hand-written helpers.
- Add focused models and convenience commands by hand; do not add `api.json`, generated endpoint modules, runtime endpoint registries, or file-scanning endpoint discovery.

## Coverage summary

- Documented operations: 600
- Explicit hand-written endpoint helpers: 16
- Raw-only documented operations: 584
...
## Local drift to verify

- [ ] `GET /api/field` exists as a local convenience path but was not present in the source documentation; verify against a live Metabase or remove the convenience helper.
...
- [ ] `GET /api/collection/trash` — `get-api-collection-trash` — Fetch the trash collection, as in `/api/collection/:trash-id`
...
- [ ] `GET /api/collection/{id}/dashboard-question-candidates` — `get-api-collection-id-dashboard-question-candidates` — Find cards in this collection that can be moved into dashboards in this collection. To be eligible, a card must only appear in one dashboard (which is also i...
- [ ] `GET /api/collection/{id}/items` — `get-api-collection-id-items` — Fetch a specific Collection's items with the following options: * `models` - only include objects of a specific set of `models`. If unspecified, returns obje...
- [ ] `POST /api/collection/{id}/move-dashboard-question-candidates` — `post-api-collection-id-move-dashboard-question-candidates` — Move candidate cards to the dashboards they appear in.

### /api/comment (0/6 explicit)

- [ ] `GET /api/comment` — `get-api-comment` — Get comments for an entity
...
- [ ] `DELETE /api/comment/{comment-id}` — `delete-api-comment-comment-id` — Soft delete a comment
- [ ] `POST /api/comment/{comment-id}/reaction` — `post-api-comment-comment-id-reaction` — Toggle a reaction on a comment
...
- [ ] `POST /api/dashboard` — `post-api-dashboard` — Create a new Dashboard.
