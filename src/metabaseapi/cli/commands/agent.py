...
def agent_execute(ctx: typer.Context, body: str = typer.Argument(..., help="Agent execute JSON object")) -> None:
    ...
    _run_and_print(_run_client_call(ctx, lambda client: _raw_agent.agent_execute(client, payload)))


@app.command("get-agent-metric")
def get_agent_metric(ctx: typer.Context, metric_id: str = typer.Argument(...)) -> None:
    _run_and_print(_run_client_call(ctx, lambda client: _raw_agent.get_agent_metric(client, metric_id)))
...
def get_agent_metric_field_values(
    ...
    _run_and_print(
        _run_client_call(ctx, lambda client: _raw_agent.get_agent_metric_field_values(client, metric_id, field_id))
    )
...
def agent_search(ctx: typer.Context, body: str = typer.Argument(..., help="Agent search JSON object")) -> None:
    payload = _parse_json_object(body, "body")
    _run_and_print(_run_client_call(ctx, lambda client: _raw_agent.agent_search(client, payload)))


@app.command("get-agent-table")
def get_agent_table(ctx: typer.Context, table_id: str = typer.Argument(...)) -> None:
    _run_and_print(_run_client_call(ctx, lambda client: _raw_agent.get_agent_table(client, table_id)))
...
def get_agent_table_field_values(
    ...
    _run_and_print(
        _run_client_call(ctx, lambda client: _raw_agent.get_agent_table_field_values(client, table_id, field_id))
    )
...
def agent_construct_query(
    ...
    payload = _parse_json_object(body, "body")
    _run_and_print(_run_client_call(ctx, lambda client: _raw_agent.agent_construct_query(client, payload)))


@app.command("agent-query")
def agent_query(ctx: typer.Context, body: str = typer.Argument(..., help="Agent query JSON object")) -> None:
    payload = _parse_json_object(body, "body")
    _run_and_print(_run_client_call(ctx, lambda client: _raw_agent.agent_query(client, payload)))
